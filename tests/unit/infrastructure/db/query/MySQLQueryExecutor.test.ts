...
describe("MySQLQueryExecutor", () => {
  ...
  let queryExecutor: MySQLQueryExecutor;
  ...
  beforeEach(() => {
    ...
    queryExecutor = new MySQLQueryExecutor(mockConnection);
  });

  describe("insert", () => {
    it("should execute INSERT query with MySQL syntax", async () => {
  ...
  describe("select", () => {
    it("should execute SELECT query with MySQL parameter syntax", async () => {
  ...
  describe("update", () => {
    it("should execute UPDATE query with MySQL parameter syntax", async () => {
      const mockResult: DatabaseResult = { rows: [], rowCount: 1 };
      mockConnection.query.mockResolvedValue(mockResult);
  ...
  describe("delete", () => {
    it("should execute DELETE query with MySQL parameter syntax", async () => {
      const mockResult: DatabaseResult = { rows: [], rowCount: 1 };
      mockConnection.query.mockResolvedValue(mockResult);
  ...
  describe("upsert", () => {
    it("should execute UPSERT query with MySQL ON DUPLICATE KEY UPDATE syntax", async () => {
      const mockResult: DatabaseResult = { rows: [], rowCount: 1 };
      mockConnection.query.mockResolvedValue(mockResult);
    ...
    it("should handle conflict-only columns gracefully", async () => {
      ...
      expect(mockConnection.query).toHaveBeenCalledWith(
        expect.stringContaining("INSERT INTO test_table (id)"),
        ["123"]
      );
  ...
  describe("selectWithJoin", () => {
    it("should execute complex JOIN query with MySQL syntax", async () => {
      const mockResult: DatabaseResult = { rows: [], rowCount: 0 };
      mockConnection.query.mockResolvedValue(mockResult);
  ...
  describe("support count operations", () => {
    ...
    it("should decrement support count with GREATEST function", async () => {
      const mockResult: DatabaseResult = { rows: [], rowCount: 1 };
      mockConnection.query.mockResolvedValue(mockResult);
